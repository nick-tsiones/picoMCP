...
export const PICO8_API: ApiFunction[] = [
  ...
  { name: "cursor", args: "x y [col]", description: "Set cursor position for print()." },
  {
    name: "fget",
    args: "n [f]",
    description: "Get sprite flag f (0-7) for sprite n. Returns all flags if f omitted.",
  },
  {
    name: "fillp",
    args: "pat",
    description: "Set fill pattern (bits 0x0-0xf). Default 0x0 (solid).",
  },
  {
    name: "fset",
    args: "n [f] v",
    description: "Set sprite flag f (0-7) for sprite n to boolean v.",
  },
  { name: "line", args: "x0 y0 x1 y1 [col]", description: "Draw a line." },
  {
    name: "map",
    args: "cel_x cel_y sx sy cel_w cel_h [layer]",
    description: "Draw a map section starting at cel_x,cel_y on screen at sx,sy.",
  },
  ...
  { name: "mset", args: "x y v", description: "Set map tile at (x, y) to value v." },
  {
    name: "pal",
    args: "[c0 c1 [p]]",
    description: "Swap color c0 to c1. pal() resets. p=0 screen, p=1 draw.",
  },
  {
    name: "palt",
    args: "[c t]",
    description: "Set color c transparency. palt() resets (black transparent).",
  },
  { name: "pget", args: "x y", description: "Get pixel color at (x, y) on sprite sheet." },
  {
    name: "print",
    args: "str [x y [col]]",
    description: "Print a string. Uses cursor if x,y omitted.",
  },
  ...
  {
    name: "spr",
    args: "n x y [w h] [flip_x] [flip_y]",
    description: "Draw sprite n at screen (x,y). w,h in sprites; default 1,1.",
  },
  { name: "sset", args: "x y [col]", description: "Set pixel on sprite sheet at (x, y)." },
  {
    name: "sspr",
    args: "sx sy sw sh dx dy [dw dh] [flip_x] [flip_y]",
    description: "Draw a region from sprite sheet (sx,sy,sw,sh) to screen (dx,dy,dw,dh).",
  },
  {
    name: "tline",
    args: "x0 y0 x1 y1 mx my [mdx mdy]",
    description: "Draw a textured line with offset (mx,my).",
  },
  ...
  // Input
  {
    name: "btn",
    args: "i [p]",
    description: "Return true if button i held by player p (default 0).",
  },
  ...
  {
    name: "music",
    args: "[n [fade_len [channel_mask]]]",
    description: "Play music n (0-63). -1 stops. fade_len in ms.",
  },
  {
    name: "sfx",
    args: "n [channel [offset [length]]]",
    description: "Play SFX n (0-63) on given channel (0-3).",
  },
  ...
  // Strings
  {
    name: "sub",
    args: "str start [end]",
    description: "Substring from start to end (1-indexed, inclusive).",
  },
  ...
  // Memory / Peek / Poke
  {
    name: "cstore",
    args: "addr val",
    description: "Write a 32-bit value to cartridge ROM. addr in 0x0000-0x0fff.",
  },
  {
    name: "memcpy",
    args: "dest src len",
    description: "Copy len bytes from src address to dest address.",
  },
  ...
  {
    name: "reload",
    args: "dest src len [filename]",
    description: "Load data from cartridge ROM into memory.",
  },
  {
    name: "cstore",
    args: "addr val",
    description: "Write 32-bit val to cartridge ROM (0x0000-0x0fff).",
  },
  ...
  // Cartridge / System
  {
    name: "cartdata",
    args: "id",
    description: "Open persistent cart data (64 numbers). Must be called once.",
  },
  ...
  {
    name: "printh",
    args: "str [filename] [overwrite] [save_to_desktop]",
    description: "Print to host console or file.",
  },
  { name: "run", args: "", description: "Restart the cartridge from _init()." },
  {
    name: "stat",
    args: "n",
    description: "Get system stat: 0=memory, 1=CPU%, 4=clipboard, 6=param, 16-19=SFX info.",
  },
  ...
  {
    name: "mapdraw",
    args: "layer cel_x cel_y sx sy cel_w cel_h [sx2 sy2]",
    description: "Map draw helper. layer 0=shared, 1=screen.",
  },
  {
    name: "tilemap_set",
    args: "map_x map_y sx sy sw sh [layer]",
    description: "Extended map drawing. layer 0=shared, 1=screen.",
  },
];
...
export const PICO8_PITFALLS: Pitfall[] = [
  {
    ...
    problem:
      "Graphics are drawn into an off-screen buffer. Without calling flip(), nothing appears on screen.",
    remedy: "Always call flip() at the end of _draw() to swap buffers.",
  },
  {
    title: "Mixed tab indentation in Lua",
    problem:
      "PICO-8 requires tabs for indentation in the code editor. Mixing spaces and tabs can cause syntax errors or unexpected token counts.",
    remedy:
      "Use only tabs for indentation. The PICO-8 editor enforces this; external editors do not.",
  },
  {
    title: "Sprite sheet / map memory sharing",
    problem:
      "The bottom half of the sprite sheet (gfx rows 64-127) shares memory with the bottom half of the map (map rows 32-63). Editing one silently changes the other.",
    remedy:
      "Be aware of the aliasing. Keep sprites in the top half (0-63) if you need a full 64-row map, or reserve map rows 32-63 for sprite overflow.",
  },
  {
    title: "Token limit (8192 tokens)",
    problem:
      "PICO-8 cartridges are limited to 8192 tokens (compressed code units). Exceeding this limit prevents the cartridge from running.",
    remedy:
      "Minify code, use shorter variable names, move data to strings or the map, and use external tools to count tokens during development.",
  },
  {
    title: "Cart RAM (cartdata) 64-number limit",
    problem:
      "cartdata() only persists 64 numbers (256 bytes). Storing more data requires encoding into fewer slots or using external storage.",
    remedy:
      "Pack booleans into bits, use one number to store multiple flags, or store strings encoded across multiple slots.",
  },
  {
    title: "Lua arrays are 1-indexed",
    problem:
      "PICO-8 Lua uses 1-based indexing for tables. Using index 0 returns nil, which can cause subtle bugs.",
    remedy:
      "Always start array indices at 1. The # operator returns the length based on 1-indexed sequences.",
  },
  {
    title: "No continue statement in Lua",
    problem:
      "PICO-8's Lua dialect does not support the continue keyword in loops. Using goto for continue-like behavior is error-prone.",
    remedy:
      "Use if/else blocks or reverse the condition to skip code execution. Avoid goto patterns when possible.",
  },
  {
    title: "Variable scoping is global by default",
    problem:
      "Variables declared without 'local' are global, which can lead to accidental overwrites and hard-to-debug issues.",
    remedy: "Always declare variables with 'local'. Use _init() to initialize globals explicitly.",
  },
  {
    title: "SFX note encoding confusion",
    problem:
      "SFX notes encode pitch, instrument, volume, and effect in compact bit fields. Manual editing without understanding the encoding corrupts SFX data.",
    remedy:
      "Use the PICO-8 SFX editor or a tool that understands the binary format. Each note is 2 bytes: key(6) + instrument(2), volume(3) + effect(3) + custom(2).",
  },
  {
    title: "Color palette indices and transparency",
    problem:
      "PICO-8 has 16 fixed colors (0-15). Color 0 (black) is transparent by default via palt(). Setting other colors as transparent can cause unexpected drawing behavior.",
    remedy:
      "Use palt() to control which colors are transparent. palt() with no args resets to default (only black transparent).",
  },
  {
    title: "Screen coordinates are 128x128 but display varies",
    problem:
      "PICO-8 draws to a 128x128 internal buffer, but display scaling can cause blurring or aspect ratio issues.",
    remedy:
      "Design for 128x128. Use clip() to restrict drawing regions. Test at 1x and scaled resolutions.",
  },
  {
    title: "String character limit in code",
    problem:
      "Long strings in PICO-8 code consume many tokens. A string of 100 characters may use 100+ tokens depending on content.",
    remedy:
      "Store large text blocks as comments or in the map data. Use short string literals and build output programmatically.",
  },
  {
    title: "No bitwise NOT (~) on 32-bit values",
    problem:
      "PICO-8 numbers are 16.16 fixed point. bnot() works on the lower 16 bits only, which can surprise developers expecting 32-bit behavior.",
    remedy:
      "Use band() with a mask to limit results to 16 bits. Test bitwise operations with known inputs.",
  },
  {
    title: "Coroutine limits",
    problem:
      "PICO-8 supports cocreate()/coresume() but only a limited number of coroutines can be active. Hitting the limit causes runtime errors.",
    remedy:
      "Limit coroutine usage. Reuse coroutines or use update-based patterns instead of yielding.",
  },
  {
    title: "Code reload during dev keeps global state",
    problem:
      "When you reload a cartridge during development, global variables from the previous run may persist, leading to inconsistent state.",
    remedy:
      "Initialize all state in _init() and call run() to reset. Be aware that external editors do not always reset state.",
  },
];
